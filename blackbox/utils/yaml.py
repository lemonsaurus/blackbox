...
def get_yaml_config(config_path: pathlib.Path) -> dict:
    ...
    # autoescape stays off: this renders YAML, not HTML, and HTML-escaping
    # would corrupt config values.
    template = Environment(undefined=StrictUndefined, autoescape=False)  # noqa: S701
    ...
    try:
        ...
        raise ImproperlyConfigured(f"There is an error in the config file: {e}") from e

    except UndefinedError as e:
        raise ImproperlyConfigured(f"Missing environment variable: {e}") from e
