...
def mock_invalid_postgres_config():
    """Mock invalid Postgres config."""
    return {"username": "lime"}


def test_can_be_instantiated_with_required_fields(mock_valid_postgres_config):
    """Test if the PostgreSQL database handler can be instantiated."""
    Postgres(**mock_valid_postgres_config)


def test_fails_without_required_fields(mock_invalid_postgres_config):
