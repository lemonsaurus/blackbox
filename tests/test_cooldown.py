...
from blackbox.utils.cooldown import should_notify
...
def test_should_we_send_notification(cooldown_config):
    ...
    assert should_notify(last_send_exact, delta) is True
    last_send_long_ago = now - delta - relativedelta(hours=1)
    assert should_notify(last_send_long_ago, delta) is True
    last_send_recent = now - delta + relativedelta(seconds=10)
    assert should_notify(last_send_recent, delta) is False
