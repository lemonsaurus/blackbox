...
def mock_cron_expression_datetime_cases():
    ...
    return [
        (
            # 04:00 - 04:59
            "* 4 * * *",
            datetime(year=2025, month=2, day=21, hour=4, minute=25, second=35, microsecond=1),
            True,
        ),
        (
            # :30
            "30 * * * *",
            datetime(year=2012, month=7, day=6, hour=9, minute=25, second=35, microsecond=1),
            False,
        ),
        (
            # :30
            "30 * * * *",
            datetime(year=2012, month=7, day=6, hour=9, minute=30, second=35, microsecond=1),
            True,
        ),
        (
        ...
        (
            # 2nd month (February)
            "* * * 2 *",
            datetime(year=2022, month=2, day=16, hour=12, minute=30, second=45, microsecond=17),
            True,
        ),
        (
            # 2nd month (February)
            "* * * 2 *",
            datetime(year=2022, month=5, day=16, hour=12, minute=30, second=45, microsecond=17),
            False,
        ),
        (
        ...
        (
            ...
            "12 3 4 12 *",
            datetime(year=2025, month=2, day=22, hour=12, minute=30, second=45, microsecond=17),
            False,
        ),
        (
            # 03:12 on December 4th
            "12 3 4 12 *",
            datetime(year=2025, month=12, day=4, hour=3, minute=12, second=45, microsecond=17),
            True,
        ),
        (
        ...
        (
            # 1st or 2nd of any month at either 02:05 or 02:10 AM
            "5,10 2 1,2 * *",
            datetime(year=2025, month=12, day=2, hour=2, minute=5, second=45, microsecond=17),
            True,
        ),
        (
            # Any Sunday between 00:00 and 06:59
            "* 0-6 * * 0",
            datetime(year=2025, month=2, day=23, hour=3, minute=12, second=45, microsecond=17),
            True,
        ),
        (
            # Any Sunday between 00:00 and 06:59
            "* 0-6 * * 0",
            datetime(year=2025, month=2, day=23, hour=6, minute=12, second=45, microsecond=17),
            True,
        ),
        (
            # Any Sunday between 00:00 and 06:59
            "* 0-6 * * 0",
            datetime(year=2025, month=2, day=23, hour=7, minute=0, second=45, microsecond=17),
            False,
        ),
    ]
...
def test_date_within_retention_days(mock_retention_days):
    ...
    for retention_days in mock_retention_days:
        assert (
            rotation.within_retention_days(
...
def test_construct_retention_tracker(mock_rotation_strategies, mock_retention_tracker):
    ...
    assert (
        rotation.construct_retention_tracker(
...
def test_meets_delete_criteria(mock_meets_delete_criteria_params):
    ...
    for params in mock_meets_delete_criteria_params:
        assert rotation.meets_delete_criteria(**params["args"]) == params["expected_response"]
